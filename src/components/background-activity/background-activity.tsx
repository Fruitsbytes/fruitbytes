...
@Component({
  tag: 'background-activity',
  styleUrl: 'background-activity.scss',
  shadow: true,
})
export class BackgroundActivity {
    ...
    await this.my3d?.init(
      {
        ...
        animation: (scene, _c, _t, _d) => {
          ...
          if (this._box?.userData.animationFinished === true) {
  ...
  @Listen('pull.box.up', { target: 'document', capture: true })
  pullBoxUp() {
    console.log('olla');
    if (this._box && this.my3d) {
      const { y } = this._box.position;
      ...
      const animationClip = new AnimationClip(undefined, 1, [track]);
      this.my3d.animateObject(this._box, animationClip, {
        onFinished: () => {
          if (this._box) this._box.userData.animationFinished = true;
          setTimeout(async () => {
            await this.startBalls();
          });
        },
      });
    }
  ...
  async startBalls() {
    if (!this.ready || this._busyBalling) {
      console.log('🍝');
    }

    this._busyBalling = true;
    setTimeout(() => {
      this._busyBalling = false;
    }, 8000);
    ...
    const code = this.digiCode.split('');

    if (this.my3d) {
      for (let b of code) {
        // const selectedMat = materialsParam[randomIntFromInterval(0, 9)]
        const objectType = b === '0' ? 'sphere' : 'bar';
        ...
        const x = getRandomArbitrary(-8, 8),
          y = 40 + getRandomArbitrary(0, 800),
          z = getRandomArbitrary(-8, 8),
  ...
  start() {
    this.startBalls().catch(e => console.error(e));
  }
  ...
  @Watch('menuWidth')
  resize(_newValue: string, _oldValue: string) {
    this._resize();
  }
  ...
  makeRoughBall(mesh: Mesh, bassFr: number, treFr: number) {
